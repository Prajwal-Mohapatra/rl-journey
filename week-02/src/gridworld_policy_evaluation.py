...
Implements the full transition model P[s][a] and iterative policy evaluation.
...
Commit: feat(w02): iterative policy evaluation on GridWorld
"""
...
class GridWorld:
    ...
    def greedy_policy_from_V(self, V: np.ndarray) -> np.ndarray:
        """
        One-step greedy improvement: for each state pick the action with the
        highest expected value under the current V.  Returns a one-hot policy.

        This is the Policy Improvement step — you'll build on it in Week 3.
        """
        actions = np.zeros(self.n_states, dtype=int)

        for s in range(self.n_states):
            if self.is_terminal(s):
                actions[s] = 0          # arbitrary — never executed
                continue

            q_values = []
            for a in range(self.n_actions):
                q = sum(
                    prob * (r + (0.0 if done else self.gamma * V[s_prime]))
                    for prob, s_prime, r, done in self.P[s][a]
                )
                q_values.append(q)

            actions[s] = int(np.argmax(q_values))

        return self.deterministic_policy(actions)
...
def build_figure(
    ...
    V: np.ndarray,
    deltas: list[float],
    save_path: str = "policy_evaluation.png",
) -> None:
    """
    Five-panel figure:
...
def main() -> None:
    ...
    build_figure(gw, V, deltas,
                 save_path="policy_evaluation.png")
